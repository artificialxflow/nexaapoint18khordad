...
import { createNoteBoard } from '@/src/lib/meizito/workspace/server';
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const noteBoard = await createNoteBoard(businessId, body.name, access.user.id);
    return jsonOk({ noteBoard }, { status: 201 });
  } catch (err) {
    return handleAuthRouteError(err, 'meizito.noteBoards.create');
  }
}
